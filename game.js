...
function Block(x,y,depth,type,movable){
    ...
    this.draw = function(){
        image(blockType[this.type], this.x-boxin.x+canvasWidth/2, this.y-boxin.y+canvasHeight/3, blockHeight, blockHeight);
    }
}
...
function mousePressed() {
    if ((Math.abs(canvasWidth/8 - mouseX)**2 + Math.abs(canvasHeight/4*3 - mouseY)**2)**0.5 < canvasHeight/10){
        ...
        loop2:
                for (var j=0; j<level[i].length; j++){
                    var ghostX = level[i][j].x - boxin.x+canvasWidth/2
                    if (ghostX < mouseX && ghostX+blockHeight >= mouseX && level[i][j].movable){
                        var ghostY = level[i][j].y - boxin.y+canvasHeight/3
                        if (ghostY < mouseY && ghostY+blockHeight >= mouseY){
                            ...
                            for (var k=0; k<level[selectedGroup].length; k++){
                                level[selectedGroup][k].initialX = level[selectedGroup][k].x
                            }
